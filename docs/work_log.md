...
| 2026-09-22 | **홈 탑이슈 출처 무결성 수정**: 기존 `daily_highlights`가 `country_codes`만 저장하고 홈 JS가 같은 국가 `top_news` 최대 4건을 제목 검증 없이 링크로 붙이던 구조를 제거. LLM 입력에 `article_id`를 표시하고 출력의 `source_article_ids`를 입력 후보 집합으로 검증한 뒤 저장하며, export가 해당 ID의 기사 카드·동일사건 링크를 직접 포함하도록 변경. 구 데이터만 유사도 폴백 사용. `display_audit.py`에 `HIGHLIGHT_SOURCE_MISSING/INVALID`를 추가하고 `deploy_web.sh`가 `--strict` 실패 시 배포를 중단하도록 변경. 실제 재생성 결과 10건의 출처 ID 무결성 통과 후, 베트남 USD 190 million을 190억 달러/$1.9bn으로 부풀린 오류를 발견해 USD 금액 근거 검증을 추가; 잘못된 항목은 제외되어 최종 9건 모두 통과. 테스트 27건 통과, 로컬 export 완료, Cloudflare 미배포. |
