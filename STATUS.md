...
- **관련 기사 링크 제목 불일치(9/22 발견·수정)**: 자기참조(같은 기사, url 동일)가 카드 헤드라인이 아닌 원문 스크래핑 제목으로 표시되어 다른 기사처럼 보임(인도 탭 '러시아 제재법' 등). `export_json.py` 수정 완료·`display_audit.py`에 SELF_TITLE_MISMATCH 점검 추가. **export+배포 미실행**, 다음 실행부터 적용.
