...
# Haiku+배치(50%↓)라 처리비용이 싸므로 하루 물량을 다 소화하게 상향.
#
# 2026-09-03 재조정 — 800 은 실제로 병목이었다.
# 일일 운용은 `python main.py ai --days 2` 라 한 번에 "최근 2일 게시분"이 대상인데,
# news.db 실측(최근 21일) 2일창 크기가 350~1,632건으로 편차가 크다:
#     8/27 1,632  8/19 1,427  8/20 1,411  8/28 1,490  9/02 1,200  …  8/16 50
# 창이 상한을 넘긴 날의 초과분은 다음 날이면 창 밖으로 밀려나 **영구 미처리**가 된다.
# 이렇게 새 나간 누적치가 21일간 4,728건(현재 총 백로그 5,739건의 대부분).
#   상한별 21일 누락:  800 → 4,728건 / 1200 → 1,218건 / 1600 → 32건
# → 1600 채택(피크일까지 흡수). 800→1600 이면 실제 처리량은 하루 평균 약 +220건.
#
# 비용(Haiku+배치 실측 기준): prefilter 약 0.3원/건, rank 약 2원/건.
#   → 평균 하루 +300원 안팎. 8일 이상 지난 기존 백로그는 --days 2 창 밖이라 건드리지 않는다
#     (오래된 뉴스라 처리할 이유도 없음 — 이 상향은 "앞으로 안 새게" 하는 조치).
#
# RANK_LIMIT: 2026-09-08 재조정 — 400은 실제로 병목이었다.
# 최근 30일 실측 2일창 keep 합계가 400을 절반 가까운 날에서 초과(예: 08/12~13
# 552건, 08/19~20 506건, 08/09~10 489건, 08/24~25 488건, 09/07~08 429건 등).
# 초과분은 PREFILTER_LIMIT과 같은 이유로 --days 2 창 밖으로 밀려나 영구 미처리됨.
# → 700 채택(실측 피크 552를 여유있게 흡수). 비용은 rank가 prefilter의 약 6배라
#   피크일에만 최대 +600원 안팎(매일 아님, 평시엔 영향 거의 없음).
PREFILTER_LIMIT = 1600       # 한 번 실행에 처리할 최대 기사 수(prefilter)
RANK_LIMIT      = 700        # 한 번 실행에 처리할 최대 기사 수(ranker)
